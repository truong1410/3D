...
SAVE_PATH = BASE + 'tnet_smooth.pth'
EPOCHS    = 50; LR = 1e-4; ALPHA = 0.1; LAMBDA_S = 0.01
...
# Load từ best model hiện tại làm điểm khởi đầu
encoder = ResnetEncoder(18, pretrained=False).to(DEVICE)
decoder = DepthDecoder(encoder.num_ch_enc).to(DEVICE)
tnet    = TNet().to(DEVICE)
ckpt    = torch.load(BASE + 'tnet_final.pth', weights_only=False, map_location=DEVICE)
encoder.load_state_dict(ckpt['encoder'])
decoder.load_state_dict(ckpt['decoder'])
tnet.load_state_dict(ckpt['tnet'])
print(f"Loaded tnet_final (AbsRel={ckpt['absrel']:.4f})")
...
for epoch in range(EPOCHS):
    ...
    total_l1 = total_lp = total_lc = total_ls = 0; nb = 0
    ...
    for batch in train_loader:
        ...
        lsmooth = smoothness_loss(depth_t, t)   # ← thêm smoothness

        loss = l1 + lp + ALPHA * lcons + LAMBDA_S * lsmooth
        ...
        total_lc += lcons.item(); total_ls += lsmooth.item()
        nb += 1
...
print(f"Compare: tnet_final={ckpt['absrel']:.4f} → tnet_smooth={best_absrel:.4f}")
