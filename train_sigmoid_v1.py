...
SAVE_PATH = BASE + 'tnet_sigmoid_v1.pth'
EPOCHS    = 50; LR = 1e-4; ALPHA = 0.1

ray_map = generate_ray_map(INTRINSICS)

# Sigmoid v1 sequences
seqs = [BASE + s for s in [
    'sigmoid_t1_a', 'sigmoid_t2_a',
    'sigmoid_t3_a', 'sigmoid_t3_b']]

full_ds = ConcatDataset([C3VDDataset(p) for p in seqs if os.path.exists(p)])
...
print(f"[sigmoid v1] Train: {n_train} | Val: {n_val} | Device: {DEVICE}")
