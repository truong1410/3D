...
sys.path.insert(0, '/mnt/data1/home/bmustudent01/workspace/nguyenhng/3D')
...
SAVE_PATH = BASE + 'mobile2frame.pth'
...
print(f"[Mobile+TNet2] Train: {n_train} | Val: {n_val}")
...
for epoch in range(EPOCHS):
    ...
    for batch in train_loader:
        ...
        torch.nn.utils.clip_grad_norm_(
            list(depthnet.parameters()) + list(tnet.parameters()), 1.0)
        optimizer.step()
    ...
    with torch.no_grad():
        for batch in val_loader:
            pred = disp_to_depth(depthnet(batch['color'].to(DEVICE))[('disp',0)])
    ...
    if avg_abs < best_absrel:
        best_absrel = avg_abs
        torch.save({'epoch': epoch, 'depthnet': depthnet.state_dict(),
                    'tnet': tnet.state_dict(), 'absrel': avg_abs}, SAVE_PATH)
