...
SAVE_PATH = BASE + 'tnet_v2combined.pth'
EPOCHS    = 50; LR = 1e-4; ALPHA = 0.1

ray_map = generate_ray_map(INTRINSICS)

# Train: clean t1/t2/t3 + debris t1/t2/t3 — giữ t4 cả hai làm val
train_seqs = [V2BASE + s for s in [
    'c2_sigmoid_t1',    'c2_sigmoid_t2',    'c2_sigmoid_t3',
    'c2_sigmoidv3_t1',  'c2_sigmoidv3_t2',  'c2_sigmoidv3_t3']]
val_seqs   = [V2BASE + s for s in [
    'c2_sigmoid_t4', 'c2_sigmoidv3_t4']]  # held-out cả clean + debris

train_ds = ConcatDataset([C3VDDataset(p) for p in train_seqs if os.path.exists(p)])
val_ds   = ConcatDataset([C3VDDataset(p) for p in val_seqs   if os.path.exists(p)])

train_loader = DataLoader(train_ds, batch_size=4, shuffle=True,  num_workers=2)
val_loader   = DataLoader(val_ds,   batch_size=4, shuffle=False, num_workers=2)
print(f"[v2 clean+debris] Train: {len(train_ds)} | Val: {len(val_ds)}")
