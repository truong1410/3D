...
SAVE_PATH = BASE + 'tnet_v2clean.pth'
EPOCHS    = 50; LR = 1e-4; ALPHA = 0.1

ray_map = generate_ray_map(INTRINSICS)

# Train: chỉ v2 clean (t1, t2, t3) — giữ t4 làm val
train_seqs = [V2BASE + s for s in [
    'c2_sigmoid_t1', 'c2_sigmoid_t2', 'c2_sigmoid_t3']]
val_seqs   = [V2BASE + 'c2_sigmoid_t4']  # held-out clean
test_seqs  = [V2BASE + s for s in [      # debris — chỉ test sau
...
print(f"[v2 clean] Train: {len(train_ds)} | Val: {len(val_ds)}")
...
print("Next: run generalization test on debris sequences")
